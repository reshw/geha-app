...
const ExpenseDetailModal = ({ expense, selectedSpace, onClose, onApprove, onReject }) => {
  ...
  const handleApprove = async () => {
    const message = expense.type === 'income'
      ? '이 입금을 승인하시겠습니까?'
      : '이 청구를 승인하시겠습니까?';
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 text-gray-700">
                ...
                <span className="font-medium">{expense.type === 'income' ? '입금일' : '사용일'}:</span>
                <span>{formatDate(expense.usedAt)}</span>
              </div>
              
              <div className="text-sm text-gray-500">
                {expense.type === 'income' ? '등록일' : '청구일'}: {formatDateTime(expense.createdAt)}
              </div>

              {/* 게스트 자동 입금 정보 */}
              {expense.type === 'income' && expense.transactionType === 'auto_guest_reservation' && expense.guestInfo && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <div className="text-sm font-semibold text-blue-800 mb-1">
                    🏠 게스트 예약 자동 생성
                  </div>
                  <div className="text-sm text-blue-700">
                    게스트: {expense.guestInfo.name}
                  </div>
                  <div className="text-xs text-blue-600 mt-1">
                    {formatDate(expense.guestInfo.checkIn)} ~ {formatDate(expense.guestInfo.checkOut)} ({expense.guestInfo.nights}박)
                  </div>
                </div>
              )}
            </div>
